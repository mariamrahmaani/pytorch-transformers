...
def train(args, train_dataset, model, tokenizer):
    ...
    train_sampler = RandomSampler(train_dataset) if hvd.size()==0 else DistributedSampler(train_dataset, num_replicas=hvd.size(), rank=hvd.rank())
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size)   

    if args.max_steps > 0:
        t_total = args.max_steps
        args.num_train_epochs = args.max_steps // (len(train_dataloader) // args.gradient_accumulation_steps) + 1
        logger.info("+++++++++++++++ (if args.max_steps > 0) args.num_train_epochs = args.max_steps // (len(train_dataloader) // args.gradient_accumulation_steps)1 t_total = %d", t_total)
    else:
        t_total = len(train_dataloader) // (args.gradient_accumulation_steps * args.num_train_epochs)
        logger.info("++++++++++++++++&&&&&& else... len(train_dataloader) = %d",  len(train_dataloader))
        logger.info("++++++++++++++++&&&&&& else... args.gradient_accumulation_steps = %d",  args.gradient_accumulation_steps)
        logger.info("++++++++++++++++&&&&&& else... args.num_train_epochs = %d",  args.num_train_epochs)
        logger.info("++++++++++++++++ else (t_total = len(train_dataloader) // (args.gradient_accumulation_steps * args.num_train_epochs) t_total = %d", t_total)
    ...
    logger.info("+++++++++++++++++++ test test test +++++++ hvd.rank() = %d", hvd.rank())
    ...
    logger.info("++++++++++++++++ else... len(train_dataloader) = %d  & hvd.rank() =%d",  len(train_dataloader), hvd.rank())
    logger.info("++++++++++++++++ else... args.gradient_accumulation_steps = %d",  args.gradient_accumulation_steps)
    logger.info("++++++++++++++++ else... args.num_train_epochs = %d",  args.num_train_epochs)
